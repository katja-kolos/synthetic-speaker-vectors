...
from train_cgan_model import main as train_model
from sample_cgan_model import _generate_artificial_embeddings
...
def find_latest_checkpoint(model_dir: Path, suffix="_wgan") -> Path:
    ...
    if not files:
        # raise FileNotFoundError(f"No cwgan checkpoint found in {model_dir}")
        print(f"No cwgan checkpoint found in {model_dir}")
        return None
    latest_file = max(files, key=lambda f: f.stat().st_mtime)
    return latest_file

def sample_vector(vectors: torch.Tensor) -> torch.Tensor:
    """Randomly sample one vector from cwgan embeddings."""
...
def generate_synthetic_embeddings(model_path: Path, output_dir: Path, n_vectors: int, gpu_id: int):
    #convention: 0, 1, 0, 1, 0, 1... labels with map_of_labels = {'m': 0, 'f': 1}
    labels = [0, 1]*(int(n_vectors/2))
    if n_vectors % 2: 
        labels += [1]
...
def create_eval_script(setting: str, vectors_file: Path, natural_data: Path, artificial_audios: Path):
    ...
    with open(script_path, "w") as f:
        f.write(f"""#!/bin/bash
echo "Evaluating embedding pool for cWGAN-{setting} vectors..."
...
def main():
    parser = argparse.ArgumentParser(
        description="Train cwgan model, sample embeddings, synthesize audio, and create eval script."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Version of the cwgan model (e.g., v0.2)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to cwgan training config (optional, auto-generated from setting if not specified)"
    )
    parser.add_argument(
        "--file-output-dir",
        type=str,
    ...
    MODEL_DIR = Path(args.model_dir) / f"cwgan-{args.setting}"
    CONFIG_PATH = Path(args.config_path) if args.config_path else Path(f"../models/cWGAN/configs/train_cwgan-{args.setting}.json")
    FILE_OUTPUT_DIR = Path(args.file_output_dir) if args.file_output_dir else Path(f"../../workspace/synthetic_speaker_embeddings/cWGAN-{args.setting}")
    ...
    setting_name = f"cwgan-{args.setting}"
