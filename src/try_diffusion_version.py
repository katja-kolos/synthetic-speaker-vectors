...
from train_diffusion_model import main as train_model
from sample_diffusion_model import _generate_artificial_embeddings
...
def run_training(args: SimpleNamespace, log_file: Path = None):
    """Run diffusion model training and optionally log output."""
    if log_file:
...
def find_latest_checkpoint(model_dir: Path, suffix="_diffusion") -> Path:
    files = list(model_dir.glob(f"*{suffix}"))
    if not files:
        raise FileNotFoundError(f"No diffusion checkpoint found in {model_dir}")
    latest_file = max(files, key=lambda f: f.stat().st_mtime)
    return latest_file

def sample_diffusion_vector(vectors: torch.Tensor) -> torch.Tensor:
    """Randomly sample one vector from diffusion embeddings."""
...
def generate_synthetic_embeddings(model_path: Path, output_dir: Path, n_vectors: int, gpu_id: int):
...
def synthesize_harvard_sentences(vectors: torch.Tensor, setting: str, do_save: str='wav'):
    ...
    for utt_id, sentence in transcripts.items():
        speaker_vector = sample_diffusion_vector(vectors)
...
def create_eval_script(setting: str, vectors_file: Path, natural_data: Path, artificial_audios: Path):
    ...
    with open(script_path, "w") as f:
        f.write(f"""#!/bin/bash
echo "Evaluating embedding pool for Diffusion-{setting} vectors..."
...
def main():
    parser = argparse.ArgumentParser(
        description="Train diffusion model, sample embeddings, synthesize audio, and create eval script."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Version of the diffusion model (e.g., v0.2)"
    )
    ...
    parser.add_argument("--data-path", type=str, default="../../workspace/selected_voice_reference_dataset/embeddings/reference_embeddings/utt-level/speaker_vectors.pt", help="Path to training embeddings")
    parser.add_argument("--model-dir", type=str, default="../../workspace/models", help="Base directory for model checkpoints")
    parser.add_argument(
        "--config-path",
        type=str,
        default=None,
        help="Path to diffusion training config (optional, auto-generated from setting if not specified)"
    )
    parser.add_argument(
        "--file-output-dir",
        type=str,
    ...
    MODEL_DIR = Path(args.model_dir) / f"diffusion-{args.setting}"
    CONFIG_PATH = Path(args.config_path) if args.config_path else Path(f"../models/Diffusion2/configs/train_diffusion-{args.setting}.json")
    FILE_OUTPUT_DIR = Path(args.file_output_dir) if args.file_output_dir else Path(f"../../workspace/synthetic_speaker_embeddings/Diffusion-{args.setting}")
    ...
    if not Path(vectors_file).exists():
        ...
        vectors, unused_indices = generate_synthetic_embeddings(model_checkpoint, FILE_OUTPUT_DIR, args.n_vectors, args.gpu_id)
    ...
    setting_name = f"diffusion-{args.setting}"
